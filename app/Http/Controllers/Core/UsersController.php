...
class UsersController extends Controller {
	...
	function postSave( Request $request, $id =0)
	{
	    ...
	    if((session()->get('gid') == 1) || (session()->get('gid') == 2 && $request->group_id >= session()->get('gid')) || (session()->get('gid') == $request->group_id))
        {
			...
			if(!is_null($request->input('groups')))
			{
				...
				for($i=0; $i<count($groups); $i++)
				{
					...
					foreach($users as $row)
					{
						...
						$data['cnf_appname'] = $this->data['tecnoconfig']['cnf_appname'];
