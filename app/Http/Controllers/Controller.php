...
abstract class Controller extends BaseController {
	...
	{	
		...
		foreach($str as $f){
			$field = $f['field'];
			// Update for V5.1.5 issue on Autofilled createOn and updated_at fields
	...
	public function getSearch( $mode = 'native' )
	{
		...
		return view('tecno.module.utility.search',$this->data);
	...
	function getImport( Request $request)
	{
		...
		if($task !='')
		{
			...
			return view('tecno.module.utility.import', $this->data);
		}	
	...
	public function getExpotion()
	{
		...
		return view('tecno.module.utility.export', $this->data);
	}
	public function getExport( Request $request, $t = 'excel')
	{
		...
		if($t == 'word')
		{			
			 return view('tecno.module.utility.word',$content);
			 
		} else if($t == 'pdf') {		
		 
		 	$html = view('tecno.module.utility.pdf', $content)->render();
	    	return \PDF::load($html)->show();
			
		} else if($t == 'csv') {		
		 
			return view('tecno.module.utility.csv',$content);
			
		} else if ($t == 'print') {
		
			//return view('tecno.module.utility.print',$content);
			$data['html'] = view('tecno.module.utility.print', $content)->render();	
			return view('layouts.blank',$data);			 
			 				 
		} else  {			
		
			 return view('tecno.module.utility.excel',$content);
		}	
	}		
	...
	function getLookup( Request $request, $id)
	{
		...
		if(count($args)>=2) 
		{
			...
			$data['key']		= $info['key'];
			//$data['ids']		= 'md'-$info['id'];
			return view('tecno.module.utility.masterdetail',$data);
