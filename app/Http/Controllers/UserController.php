...
class UserController extends Controller {
	...
	public function postSignin( Request $request) {
		...
		if($this->config['cnf_recaptcha'] =='true') $rules['captcha'] = 'required|captcha';
		$validator = Validator::make($request->all(), $rules);
		if ($validator->passes()) {	
	...
	public function postRequest( Request $request)
	{
		...
		if ($validator->passes()) {	
			...
			if($user->count() >=1)
			{
				...
				$subject = "[ " .$this->config['cnf_appname']." ] PETICIÓN PARA RECUPERACIÓN DE LA CONTRASEÑA"; 
				...
				{ 
					...
					\Mail::send('user.emails.auth.reminder', $data, function ($message) use ($data)  {
						$message->from($this->config['cnf_email'], $this->config['cnf_appname']);
			    		$message->to($data['email'])->subject($data['subject']);
			    	});	 
