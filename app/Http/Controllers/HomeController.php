...
class HomeController extends Controller {
	...
	public function  getSkin($skin='tecno')
	{
	...
	function postProccess( Request $request , $formID )
	{
		...
		if ($validator->passes()) {
			...
			if($row->method =='table')
			{
				...
				//return view('core.forms.templateforms', $data); 				
				\Mail::send('core.forms.templateforms', $data, function ($m) use ($data) {
		    		$m->to($data['email'])->subject('Submited Form :  '. $data['name']);
		    	});
