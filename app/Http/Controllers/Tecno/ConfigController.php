<?php namespace App\Http\Controllers\tecno;
...
class ConfigController extends Controller {
	...
	{	
        ...
        $this->data['pageNote']  =   \Lang::get('core.t_generalsettingsmall');	
		$this->data['active'] = '';
		return view('tecno.config.index',$this->data);	
	}
		...
		if (!$validator->fails()) 
		{
			...
			if(!is_null( $request->file('logo')))
			{
				...
				$file =  $request->file('logo'); 
			 	$destinationPath = public_path().'/assets/plugins/images/'; 
...
$val  .= 	"];\n";
			...
			$filename = base_path().'/config/tecno.php';
			$fp=fopen($filename,"w+"); 
			fwrite($fp,$val); 
			fclose($fp); 
			return Redirect::to('tecno/config')->with('messagetext','Setting Has Been Save Successful')->with('msgstatus','success');
		} else {
			return Redirect::to('tecno/config')->with('messagetext', 'The following errors occurred')->with('msgstatus','success')
	...
	public function getEmail()
	{
		...
		return view('tecno.config.email',$this->data);		
	...
	function postEmail( Request $request)
	{
		...
		if ($validator->passes()) 
		{
			...
			return Redirect::to('tecno/config/email')->with('messagetext', 'Email Has Been Updated')->with('msgstatus','success');	
			
		}	else {

			return Redirect::to('tecno/config/email')->with('messagetext', 'The following errors occurred')->with('msgstatus','success')
			->withErrors($validator)->withInput();
		}
	...
	public function getSecurity()
	{
		...
		return view('tecno.config.security',$this->data);		
...
$val  .= 	"];\n";
			...
			fwrite($fp,$val); 
			fclose($fp); 
			return Redirect::to('tecno/config/security')->with('messagetext','Setting Has Been Save Successful')->with('msgstatus','success');
		} else {
			return Redirect::to('tecno/config/security')->with('messagetext', 'The following errors occurred')->with('msgstatus','error')
			->withErrors($validator)->withInput();
		}	
	}
	...
	public function getLog( $type = null)
	{
		...
		return view('tecno.config.log',$this->data);	
	}
	...
	public function getClearlog()
	{
		...
		//return Redirect::to('tecno/config/log')->with('messagetext','Cache has been cleared !')->with('msgstatus','success');	
	}
	...
	public function getTranslation( Request $request, $type = null)
	{
		...
		return view('tecno.config.translation.'.$template,$this->data);	
	}
	
	public function getAddtranslation()
	{
		return view("tecno.config.translation.create");
	} 
	...
	public function postAddtranslation( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			return Redirect::to('tecno/config/translation')->with('messagetect','New Translation has been added !')->with('msgstatus','success');	;			
			
		} else {
			return Redirect::to('tecno/config/translation')->with('messagetext','Failed to add translation !' )->with('msgstatus','error')->withErrors($validator)->withInput();
		}		
	...
	//	$filename = 'lang.php';
		...
		fwrite($fp,$form); 
		fclose($fp); 	
		return Redirect::to('tecno/config/translation?edit='.$lang.'&file='.$file)
	...
	public function getRemovetranslation( $folder )
	{
		...
		return Redirect::to('tecno/config/translation')->with('messagetext','Translation has been removed !')->with('msgstatus','success');	
