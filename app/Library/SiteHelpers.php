...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	public static function toMasterDetail( $info )
	{
	 	...
	 	\$this->data['subform'] = \$this->detailview(\$this->modelview ,  \$this->info['config']['subform'] ,\$id );";
		 	...
		 	$tpl = array();
		 	require_once('../resources/views/tecno/module/template/native/masterdetailform.php');
	...
	static public function showNotification()
	{
		...
		<script type="text/javascript">
            $(document).ready(function(){
		        $.toast({
		            heading: '<?php echo Session::get('msgstatus');?>',
		            text: '<?php echo Session::get('messagetext');?>',
		            position: 'top-right',		           
	...
	public static function formatRows( $value ,$attr , $row = null )
	{
		...
		if($format_as =='image')
		{
			...
			if($format_value =='')
			{
				if($tecnoconfig['cnf_date'] !='' )
				{
					$value = date("".$tecnoconfig['cnf_date']."",strtotime($value));
				} 
			} else {
	...
	{
		$path = base_path().'/resources/views/tecno/module/template/';
		$fld = scandir($path);
	...
	public static function BBCode2Html($text) {
		...
		$emotion =  URL::to('tecno/js/plugins/markitup/images/emoticons/');
