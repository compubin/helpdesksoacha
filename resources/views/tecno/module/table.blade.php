...
@endif
 ...
 {!! Form::open(array('url'=>'tecno/module/savetable/'.$module_name, 'class'=>'form-horizontal','id'=>'fTable')) !!}
<div class="box box-primary">
 <div class="box-header with-border"> <h4>  {{ $row->module_title }} <small> : Table Editor ( Edit Table Setting ) </small></h4></div>
	<div class="box-body">	
		@include('tecno.module.tab',array('active'=>'table','type'=>$type))
			...
			<?php usort($tables, "SiteHelpers::_sort"); ?>
			  ...
			  <tr >
				...
				<span class=" xlick " title="Lookup Display" 
					onclick="TecnoModal('{{ URL::to('tecno/module/conn/'.$row->module_id.'?field='.$rows['field'].'&alias='.$rows['alias']) }}' ,' Connect Field : {{ $rows['field']}} ' )"
					>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    <?php echo TecnoHelpers::sjForm('fTable'); ?>
