...
<div class="box box-primary">
	...
	<div class="box-body"> 	
		...
		<ul class="parsley-error-list">
		 ...
		 {!! Form::open(array('url'=>'tecno/rac/save?return='.$return, 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) !!}
<div class="col-md-12">
	 ...
	 {!! Form::hidden('created', $row['created']) !!}					
			...
			<div style="clear:both"></div>	
				  ...
				  <div class="form-group">
					...
					<button type="button" onclick="location.href='{{ URL::to('tecno/rac?return='.$return) }}' " class="btn btn-warning btn-sm "><i class="icon-cancel-circle2 "></i>  {{ Lang::get('core.sb_cancel') }} </button>
					</div>	  
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#apiuser").jCombo("{!! url('tecno/rac/comboselect?filter=tb_users:id:email') !!}",
		{  selected_value : '{{ $row["apiuser"] }}' });
		
		$("#modules").jCombo("{!! url('tecno/rac/comboselect?filter=tb_module:module_name:module_title&limit=WHERE:module_type:!=:core') !!}",
		{  selected_value : '{{ $row["modules"] }}' });
		 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("tecno/rac/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
