...
@section('content')
<script type="text/javascript" src="{{ asset('assets/plugins/bower_components/nestable/jquery.nestable.js') }}"></script>
<link href="{{ asset('assets') }}/plugins/bower_components/nestable/nestable.css" rel="stylesheet" type="text/css" />
...
<div class="box box-primary">
  <div class="box-body"> 
	...
	<ul class="nav nav-tabs" style="margin:10px 0;">
		<li @if($active == 'top') class="active" @endif ><a href="{{ url('tecno/menu?pos=top')}}"> {{ Lang::get('core.tab_topmenu') }} </a></li>
		<li @if($active == 'sidebar') class="active" @endif><a href="{{ url('tecno/menu?pos=sidebar')}}"> {{ Lang::get('core.tab_sidemenu') }}</a></li>	
	</ul> 
  ...
  <p> {{ Lang::get('core.t_tipsdrag') }}</p>	
			...
			@foreach ($menus as $menu)
				  <li data-id="{{$menu['menu_id']}}" class="dd-item dd3-item">
					<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{$menu['menu_name']}}
						...
						<a href="{{ URL::to('tecno/menu/index/'.$menu['menu_id'].'?pos='.$active)}}"><i class="fa fa-edit"></i></a></span>
					</div>
							...
							@foreach ($menu['childs'] as $menu2)
							 <li data-id="{{$menu2['menu_id']}}" class="dd-item dd3-item">
								<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{$menu2['menu_name']}}
									<span class="pull-right">
									<a href="{{ URL::to('tecno/menu/index/'.$menu2['menu_id'].'?pos='.$active)}}"><i class="fa fa-edit"></i></a></span>
								</div>
									...
									@foreach($menu2['childs'] as $menu3)
									 	<li data-id="{{$menu3['menu_id']}}" class="dd-item dd3-item">
											<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{ $menu3['menu_name'] }}
												<span class="pull-right">
												<a href="{{ URL::to('tecno/menu/index/'.$menu3['menu_id'].'?pos='.$active)}}"><i class="fa fa-edit"></i></a>
												</span>
											</div>
										</li>	
									@endforeach
		 ...
		 {!! Form::open(array('url'=>'tecno/menu/saveorder/', 'class'=>'form-horizontal','files' => true)) !!}	
 ...
 <p> {{ Lang::get('core.t_tipsnote') }}	</p>
		...
		<div class="col-sm-7">
		 ...
		 {!! Form::open(array('url'=>'tecno/menu/save/', 'class'=>'form-horizontal','files' => true  , 'parsley-validate'=>'','novalidate'=>' ')) !!}
				...
				<input type="hidden" name="menu_id" id="menu_id" value="{{ $row['menu_id'] }}" />									
				  ...
				  <div class="form-group  " >
					...
					<div class="col-md-8">
					  ...
					  <p> View Icon Codes : 
					  <a href="{{ url('tecno/menu/icon')}}" onclick="TecnoModal(this.href,'Select Icon'); return false;"> Browse Icons  </a>  
					 </div> 
				  </div> 					
				  <div class="form-group  " >
			  ...
			  <div class="form-group">
				...
				@if($row['menu_id'] !='')
					<button type="button"onclick="TecnoConfirmDelete('{{ url('tecno/menu/destroy/'.$row['menu_id'].'?pos='.$active)}}')" class="btn btn-danger ">  Delete </button>
				@endif	
				</div>	  
