...
<div class="box box-primary">
	<div class="box-header with-border">
		...
		<div class="box-header-tools pull-left" >
			@if($access['is_add'] ==1)
	   		<a href="{{ url('velocidadsistemaoperativocontroller/update?return='.$return) }}" class="tips btn btn-success btn-circle btn-sm"  title="{{ Lang::get('core.btn_create') }}">
			...
			@endif 
			<a href="{{ url( 'velocidadsistemaoperativocontroller/search?return='.$return) }}" class="btn btn-warning btn-sm btn-circle" onclick="TecnoModal(this.href,'Advance Search'); return false;" title="{{ Lang::get('core.btn_search') }}"><i class="fa  fa-search"></i> </a>				
	...
	<div class="box-body "> 	
	 ...
	 {!! Form::open(array('url'=>'velocidadsistemaoperativocontroller/delete/', 'class'=>'form-horizontal' ,'id' =>'TecnoTable' )) !!}
    ...
    <table class="table table-bordered table-hover " id="{{ $pageModule }}Table">
        ...
        <tbody>        						
            @foreach ($rowData as $row)
                <tr>
					<td width="30"> {{ ++$i }} </td>
					<td width="50"><input type="checkbox" class="ids" name="ids[]" value="{{ $row-> }}" />  </td>
					<td>
					 	<div class="dropdown">
						  ...
						  <ul class="dropdown-menu">
							...
							<li><a href="{{ url('velocidadsistemaoperativocontroller/show/'.$row->.'?return='.$return)}}" class="tips" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search "></i> {{ Lang::get('core.btn_view') }} </a></li>
							@endif
							@if($access['is_edit'] ==1)
							<li><a  href="{{ url('velocidadsistemaoperativocontroller/update/'.$row->.'?return='.$return) }}" class="tips" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit "></i> {{ Lang::get('core.btn_edit') }} </a></li>
							@endif
						  </ul>
						</div>
...
$(document).ready(function(){
	...
	$('.copy').click(function() {
		...
		{
				$('#TecnoTable').attr('action','{{ url("velocidadsistemaoperativocontroller/copy")}}');
